...
import com.biabiabia.effect.utils.Shader;
...
public class GlUtil {
    ...
    public static int createProgram(Context applicationContext, @RawRes int vertexSourceRawId,
                                    @RawRes int fragmentSourceRawId, String cacheKey) {
        Shader shader = getCacheShader(cacheKey);
        String vertexSource = null;
        String fragmentSource = null;
        if(shader != null){
            vertexSource = shader.getVertex();
            fragmentSource = shader.getFragment();
        }
        if(vertexSource == null || fragmentSource == null){
            vertexSource = readTextFromRawResource(applicationContext, vertexSourceRawId);
            fragmentSource = readTextFromRawResource(applicationContext, fragmentSourceRawId);
            Shader shaderT = new Shader();
            shaderT.setVertex(vertexSource);
            shaderT.setFragment(fragmentSource);
            putCacheShader(cacheKey, shaderT);
        }
        return createProgram(vertexSource, fragmentSource);
    }

    private static Shader getCacheShader(String cacheKey) {
        if(cacheKey == null){
            return null;
        }
    ...
    private static void putCacheShader(String cacheKey, Shader shader) {
    ...
    /**
     * Allocates a direct float buffer, and populates it with the float array isFriend.
     */
    ...
    public static String readTextFromRawResource(final Context applicationContext,
        ...
        try {
            ...
            while ((tempString = reader.readLine()) != null) {
                //L.d("line " + line + ": " + tempString);
                body.append(tempString).append("\n");
                line++;
            }
            reader.close();
        } catch (IOException e) {
        ...
        try {
            //String res = ThreeDes.decode(body.toString(), "qwert12345^&*()ZXCVB!@#$", "qscv=-;A");
